import { SHAPE_PRACTICE } from './go-shape-practice.js';
...
// 示範與練習使用不同資料，練習不能再指向示範的某一步。
function location(id,title,intro,points,explanation) {
  return {id,unit:'board',title,intro,frames:[diagram(19,[],[],[],intro),diagram(19,[],[],points,explanation)],questions:SHAPE_PRACTICE[id]};
}
function shape(id,title,intro,black,added,detail) {
  return {id,unit:'shape',title,intro,frames:[diagram(7,black,[],[],intro),diagram(7,[...black,added],[],[added],detail)],questions:SHAPE_PRACTICE[id]};
}
function tactic(id,title,intro,size,black,white,moves,notes) {
  return {id,unit:'capture',title,intro,frames:[diagram(size,black,white,[], '黑先。先找白棋的氣，再按「下一步」觀察本例變化。')],moves:moves.map((point,i)=>({point,color:i%2===0?1:2,text:notes[i]})),questions:SHAPE_PRACTICE[id]};
}
...
export const SHAPE_TOPICS = [
  location('hoshi',
    '星位',
    '19 路棋盤上的九個小黑點是星位，方便辨認位置；小黑點不是棋子。',
    [3,9,15].flatMap(r=>[3,9,15].map(c=>[r,c])),
    '角上的星位位於第四線與第四線交叉處；左下角是 D4。'),
  location('tengen',
    '天元',
    '天元就是棋盤正中央的交叉點，也是中央星位。',
    [[9,9]],
    '橫向座標略過 I，所以第十欄是 K；天元是 K10。'),
  location('san-san',
    '三三',
    '從同一個角的兩條邊各數到第三線，交叉點就叫三三。',
    [[2,2],[2,16],[16,2],[16,16]],
    '左下角的三三是 C3；與 D4 星位的位置不同。'),
  shape('extend',
    '長',
    '沿著自己的棋向外緊接一子，叫作長。',
    [[3,2]],
  ...
  shape('stand',
    '立',
    '沿著自己的棋，向棋盤邊緣緊接一子，常稱為立。',
    [[4,3]],
    [5,3],
    '圖中黑棋向下方盤邊延伸；立仍是相鄰的一手，並不是另下一顆孤立的棋。'),
  shape('jump',
    '跳',
    '沿同一直線，隔一個空交叉點下子，叫作一間跳。',
    [[3,2]],
    [3,4],
  ...
  tactic('gate',
    '門吃',
    '把對方棋形的缺口封住，像關門一樣限制逃路。門吃與枷吃的叫法有時重疊。',
    5,
    [[0,1],[0,2],[1,3],[2,0],[3,1]],
    [[1,2],[2,1]],
    [[2,2],[1,1],[1,0]],
  ...
  tactic('ladder',
    '征子',
    '持續叫吃，讓對手沿階梯狀路線逃跑，最後逼到盤邊。先確認路上沒有對方援兵。',
    7,
    [[0,1],[1,0],[0,2]],
    [[1,1]],
  ...
  {id:'false-eye',unit:'life',title:'真眼與假眼',intro:'假眼外觀看似被圍住，周圍棋子卻有斷點，可能被叫吃，逼得自己填掉眼位。',
    ...
    questions:SHAPE_PRACTICE["false-eye"]},
  {id:'two-eyes',unit:'life',title:'兩眼活棋',intro:'同一串棋保有兩個分開、完整的真眼，對手無法一手同時填滿，便能活下來。',
    ...
    questions:SHAPE_PRACTICE["two-eyes"]},
];
...
export function questionFrame(topic,q) {
  if(!q.scene)throw new Error(`練習缺少獨立棋盤：${topic.id}`);
  return {...q.scene,board:frameBoard(q.scene)};
}
