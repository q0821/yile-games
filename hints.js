import { EMPTY, opponent, getNeighbors, getGroup, tryPlaceStone } from './rules.js';
...
export function getGamePhase(moveCount, size) {
...
export function getGuidanceLabel(x, y, rank, phase, ctx) {
  ...
  if (phase === 'opening') {
    if (isCorner(x, y)) {
      const nearby = getNeighbors(size, x, y).some(([nx, ny]) => board[nx][ny] !== EMPTY);
      return nearby ? '守角' : '佔角';
    }
...
export function buildGuidanceLegendHTML(guidanceHints, ctx) {
...
export function renderGuidanceLegend(guidanceHints, ctx) {
...
export const GoHints = {
  getCaptureHints, getGamePhase, getGuidanceLabel,
  buildGuidanceLegendHTML, renderGuidanceLegend
};
