...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('#gggCommentZh').innerText(), /黑先/);
    assert.equal(await page.locator('#gggComment').isVisible(), false);
    await page.getByText('查看英文原文', { exact: true }).click();
    assert.match(await page.locator('#gggComment').innerText(), /Black to play/);
    assert.equal(await page.locator('.ggg-board-stage canvas').count(), 1);
    ...
    assert.match(await page.locator('#gggCommentZh').innerText(), /這樣也對/);
    await page.getByText('查看英文原文', { exact: true }).click();
    ...
    await stale.p.locator('#tsumegoScreen').getByRole('button', { name: '回首頁', exact: true }).click();
