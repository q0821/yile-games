...
> **現況校正（2026-06-05）— 本文件多處為早期規劃，與實際出貨有出入，請以下列為準：**
> - **AI 引擎是 GnuGo 3.9.1（WebAssembly）**，不是 MCTS（下方概述舊敘述已修正）。約 5–6 級。
> - **Phase 9（AI 覆盤分析）/ Phase 10（新手引導）：曾實作、後又移除**。原因：GnuGo `score()` 失效、
>   棋力不足，評估會誤導使用者，已於 v2026.06.02 ~ 06.03「誠實化」砍除（見 `CHANGELOG.md`）。
> - **Phase 11（線上對戰，Firebase）：尚未實作**，仍為構想。
> - **死活練習模式（Tsumego）**：本計畫未涵蓋，後來另以 `HANDOFF_TSUMEGO.md` 規劃並出貨（趙治勳死活百科數千題）。
> - 後續優化方向另見 `SPEC_IMPROVEMENTS.md`（依序執行的待辦 spec）。

## 專案概述

純前端網頁圍棋遊戲，支援人 vs 人 / 人 vs 電腦對弈。AI 採用 **GnuGo 3.9.1（WebAssembly）**，達業餘 5–6 級水準，所有運算在瀏覽器本地完成，無需後端伺服器。另規劃（**尚未實作**）以 Firebase Realtime Database 做線上即時對戰。
...
- [x] 勝負判定
- [x] 規則選擇（中國規則 / 日本規則）
- [ ] 線上即時對戰（Firebase Realtime Database）　← 尚未實作
...
### Phase 9：AI 覆盤分析　〔⚠️ 已實作後移除 v2026.06.02~03〕

> 此階段曾完整實作（失目評分、形勢曲線、AI 建議手），但因 GnuGo `score()` 失效、
> 評估誤導使用者而全數移除。**覆盤本身（純回看）保留**。詳見 `CHANGELOG.md`。
...
### Phase 10：新手引導模式　〔⚠️ 已實作後移除 v2026.06.03〕

> 此階段曾實作（即時 AI 推薦點、定石提示），但等於教人模仿不強的 AI、且讓人不思考，已移除。
> 詳見 `CHANGELOG.md`。
