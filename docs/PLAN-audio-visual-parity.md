...
- [x] 圍棋：`GoSound.playSound('place')` 等呼叫點改 `playSfx('stone-place')`（main.js:308-309、345 等處）；sound.js 保留供 fallback
- [x] 語音觸發點依 PRD §4.2：check 接 flashCheck 路徑、mate 接 showEnd 依終局原因（ffish checkmate 判定；認輸/和局/逾時不播）
- [x] 三棋 AI 落子也要有音（AI move 套用處）
- [x] `npx jest` 全綠；`npx vite` 手動煙霧測試六棋（記錄於 commit message body）
- [x] Commit: `六棋接上音效與將軍語音`

### Task 6: BGM 接線

**Files:** Modify: `main.js`

- [x] 解鎖後若 `musicOn` 自動 startMusic；設定切換即時起停（audio-manager 內建，確認接妥；main.js 已於 Task 4 呼叫 `initAudio()`，本任務僅驗證無需改動，併入 Task 5 commit）
- [x] Commit: 併入前一個 commit（無獨立改動）
...
- [x] 棋盤：canvas 繪製細木紋/紙紋（低對比 procedural 紋理或既有 webp 疊圖）＋盤側厚度立面＋桌面投影＋外圍 vignette
- [x] 棋子：stone.js 光澤/投影微調；象/將棋子淺浮雕（內陰影＋邊緣高光）；西洋棋 glyph 加投影
- [x] 落子 scale-in 動畫（requestAnimationFrame 僅事件觸發）、最後一手標記美化、吃子淡出
- [x] 按鈕/modal/狀態列層次間距與按壓回饋統一；過場動效；`prefers-reduced-motion` 全部停用動畫
- [x] 三斷點（375/768/1280）截圖目視檢查六棋
- [x] `npx jest` 全綠
- [x] Commit: `畫面質感精修：棋盤棋子光影與介面打磨`
