...
> 整理自 2026-06 連續導入 **象棋 / 象棋殘局 / 黑白棋 / 五子棋 / 日本將棋**、
> 以及 2026-07 導入 **連六棋（Connect6）** 的實作經驗，
...
2. **`main.js`** — 四個點：
   ...
   - `HOME_ITEMS` 加一筆 `{ id, title, desc, hash: '#<game>', img: 'img/cards/<game>.webp' }`（順序＝首頁排序；`img`＝卡片封面，見 §6.1）。GPL 引擎或 iOS 不收錄的棋再加 `webOnly: true`；**自寫零 GPL 的棋不要加**，才會一併進 iOS 版（連六棋即以此補 iOS 拔掉 GPL 棋後的空缺）。
...
首頁每張卡片一張直式封面（`public/img/cards/<game>.webp`），由 `HOME_ITEMS[].img` 指向、`main.js` 的
卡片迴圈自動渲染（新增只要放檔＋填 `img`，不用改渲染碼）。**新棋封面務必比照現有卡片的水墨調性**，
否則首頁一整排會不協調。

**構圖鐵律**（對照 `gomoku.webp` / `play.webp`）：
- **尺寸 800×1200**（直式 2:3）、輸出 **webp**。
- **頂部約 45% 留白**：只有淡宣紙霧氣、不放主體——卡片標題與對句會疊在這裡，塞滿會擋字（這是最常犯的錯）。
- 主體（棋盤／棋子）壓在**下半部**，低斜視角受光、沒入霧中。
- 該棋的代表棋形當**視覺焦點**（連六棋＝一條六連黑子、五子棋＝五連、黑白棋＝夾吃佈局）。
- **淡墨近單色**、暖褐 sepia、大量留白，像褪色古卷；**不要**高飽和、不要塞滿。竹影／遠山可點綴角落。
- **無文字、無邊框、無浮水印、無 emoji**；人物一律亞洲面孔／題材。

**產法**（連六棋實作走法）：用 `gpt-image-bridge` skill 生 1024×1536 PNG →
`sips -z 1200 800` 縮圖 → `cwebp -q 82` 轉 webp（留白多的圖檔案自然偏小，40–90k 都正常）→
落 `public/img/cards/<game>.webp`。封面獨立於棋邏輯，最後補即可；不理想就重生 prompt。
...
- **`.gomoku-screen` 是六棋共用的版面 class（單欄置中），但只定義在 `style.css` 一處**：`#<game>Screen`
  掛這個 class 才會 `flex-direction:column`＋`max-width` 置中。曾有 commit 連帶刪掉這段 → gomoku/象棋/將棋/
  西洋棋/黑白棋/連六棋**全部跑版**（畫面擠成細長條、文字直排），且六棋一起壞很容易誤判成新棋的 bug。
  徵兆：`getComputedStyle(screen).flexDirection === 'row'`。改 `style.css` 檔尾遊戲版面區塊時要特別小心。
- **pvc AI 思考中離開畫面再回來會卡死**：`maybeAiMove` 的 `setTimeout` 遇 `!isActive()` 會放棄該回合，
  但 `enter<Game>Mode` 重入既有對局只 `render()`、不重排 AI → currentPlayer 卡在 AI 色、玩家點擊被 guard 擋。
  修法：重入既有 board 時補呼 `maybeAiMove()`，並在 `maybeAiMove` 開頭加 `if (aiBusy) return` 防重排
  （五子棋/黑白棋/連六棋皆已修）。
...
| 連六棋 | `#connect6` | B（自寫貪婪雙落子） | `connect6-ai/mode/rules/ui.js` | 19 路、每手 2 子（首手 1 子）、連 6 勝；重用 gomoku `candidates`；零 GPL 故 iOS 納入（不掛 webOnly） |
