...
test.each(['invalid','{"version":9}',JSON.stringify({version:2,records:{[id]:{solved:'yes'}},attempts:{}})])('損壞或未知版本不允許覆寫：%s',raw=>{
...
test('保存實際落點，重整保留非示範答案，重做清除落點',()=>{
 const s=storage(),qid='knight-practice-1';let p=beginShapeAttempt(newShapeProgress(),qid);
 p=answerShape(p,qid,'correct',[6,0]);saveShapeProgress(s,p);const loaded=readShapeProgress(s).progress;
 expect(loaded.attempts[qid].point).toEqual([6,0]);expect(beginShapeAttempt(loaded,qid).attempts[qid].point).toBeUndefined();
});
...
test('舊教室紀錄保留，不算作新題已解出',()=>{
 const s=storage();const old=JSON.stringify({version:1,topic:'knight',records:{'knight-name':{solved:true,review:false}},attempts:{}});
 s.setItem('gogame_shape_progress_v1',old);const loaded=readShapeProgress(s);expect(loaded.writable).toBe(true);expect(Object.keys(loaded.progress.records)).toHaveLength(0);
 saveShapeProgress(s,answerShape(beginShapeAttempt(loaded.progress,id),id,'correct'));
 expect(s.getItem('gogame_shape_progress_v1')).toBe(old);expect(s.getItem('gogame_shape_progress_v2')).not.toBeNull();
});
